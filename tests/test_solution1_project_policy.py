...
class TestPolicy(TestCase):
    ...
    1 - Create new Instances with 'project=vahalla' tag
    2 - Start & Stop instances tagged 'project=vahalla'
    ...
    6 - Prevent adding tags to existing instances without 'project=vahalla'
    7 - Prevent starting not 'project=vahalla'
    """
    ...
    def setUp(self):
        ...
        self.ami = self.get_export("default-ami")
        self.security_group = self.get_export("test-sg")
        # Blocked instance without tag
        self.blocked_instance = self.get_export("blocked-instance")
        # Valid instance with tag: project = vahalla
        self.valid_instance = self.get_export("valid-instance")

    # Validates simple access to Read
    ...
    def test_run_instance(self):
        ...
        try:
            self.client.run_instances(
                ...
                TagSpecifications=[
                    {
                        "Tags": [{"Key": "project", "Value": "vahalla"}],
    ...
    def test_run_instance_invalid_tag_name(self):
        ...
        try:
            self.client.run_instances(
                ...
                TagSpecifications=[
                    {
                        "Tags": [{"Key": "wrong-project", "Value": "vahalla"}],
                        "ResourceType": "instance",
                    }
                ],
                DryRun=DRY_RUN,
    ...
    def test_prevent_adding_tags_to_existing_instances(self):
        """
        6 - Prevent adding tags existing instances without 'project=vahalla'
        """
        blocked = self.ec2.Instance(self.blocked_instance)
        try:
            blocked.create_tags(
                DryRun=DRY_RUN, Tags=[{"Key": "project", "Value": "vahalla"}]
            )
        except botocore.exceptions.ClientError as e:
            error_message = e.response["Error"]["Code"]
            dry_run = "UnauthorizedOperation" == error_message
